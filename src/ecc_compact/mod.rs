    ...
    fn try_from(input: &[u8]) -> Result<Self> {
        if input.len() == PUBLIC_KEY_LENGTH {
            ...
            let public_key = Option::from(p256::AffinePoint::from_encoded_point(&encoded_point))
                .and_then(|affine_point: p256::AffinePoint| Some(affine_point.to_compact_encoded_point()))
                .and_then(|compact_point| {
                    Option::from(p256::PublicKey::from_encoded_point(&compact_point.unwrap()))
                });
...
mod tests {
    ...
    fn ecdh_interop() {
        ...
        assert_eq!(shared_secret.raw_secret_bytes().as_slice(), OTHER_SHARED_SECRET);
    }
}
